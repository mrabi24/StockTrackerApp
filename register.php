...
if (isset($_POST['submit'])) {
    ...
    } elseif (!preg_match('/^[a-zA-Z0-9_-]{3,20}$/', $_POST['firstName'])) {
    ...
    } elseif (preg_match("/^[A-Z][-'a-zA-Z]+,?\s[A-Z][-'a-zA-Z]{0,19}$/", $_POST['lastName'])) {
...
<body>
    ...
    <section class="container container-sm section-register">
            ...
            py-3">
                <div class="col form-floating">
                    <input type="text" id="username"
                        class="form-control <?php echo (!empty($usernameErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" minlength="2" maxlength="15" name="username">
                    <label for="username" class="px-4">Username</label>
                    <small <?php echo (!empty($usernameErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($usernameErr)) ? "{$usernameErr}" : "Only letters and one mandatory digit, 2 to 15 character limit." ?></small>
                </div>
            </div>
            <div class="row py-3 gx-2">
                <div class="col form-floating">
                    <input type="text" id="firstName"
                        class="form-control <?php echo (!empty($firstNameErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" name="firstName" minlength="2" maxlength="15">
                    <label for="firstName" class="px-4">First Name</label>
                    <small <?php echo (!empty($firstNameErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($firstNameErr)) ? "{$firstNameErr}" : "Max of 15 characters." ?></small>
                </div>
                <div class="col form-floating">
                    <input type="text" id="lastName"
                        class="form-control <?php echo (!empty($lastNameErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" name="lastName" minlength="2" maxlength="25">
                    <label for="retypePassword" class="px-4">Last Name</label>
                    <small <?php echo (!empty($lastNameErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($lastNameErr)) ? "{$lastNameErr}" : "Max of 25 characters." ?></small>
                </div>
            </div>
            ...
            <div class="row py-3 gx-2">
                <div class="col form-floating">
                    <input type="password" id="password"
                        class="form-control <?php echo (!empty($passwordErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" name="password">
                    <label for="password" class="px-4">Enter Password</label>
                    <small <?php echo (!empty($passwordErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($passwordErr)) ? "{$passwordErr}" : "One capital letter, and max 3 digits.  Min. 4 Characters." ?></small>
                </div>
                <div class="col form-floating">
                    <input type="password" id="retypePassword"
                        class="form-control <?php echo (!empty($passwordErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" name="retypePassword">
                    <label for="retypePassword" class="px-4">Re-enter Password</label>
                    <small <?php echo (!empty($passwordErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($passwordErr)) ? "{$passwordErr}" : "" ?></small>
                </div>
            </div>
            ...
            <div class="row py-3">
                <div class="col form-floating">
                    <input type="email" id="email"
                        class="form-control <?php echo (!empty($emailErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" name="email">
                    <label for="email" class="px-4">E-mail</label>
                    <small <?php echo (!empty($emailErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($emailErr)) ? "{$emailErr}" : "Ex:
                           example@email.com" ?></small>
                </div>
            </div>
            <div class="row py-3">
                <div class="col form-floating">
                    <input type="text" id="address"
                        class="form-control <?php echo (!empty($addressErr)) ? 'is-invalid' : ''; ?>" required
                        placeholder="" name="address">
                    <label for="address" class="px-4">Address</label>
                    <small <?php echo (!empty($addressErr)) ? "class='invalid-feedback'" : "" ?>><?php
                           echo (!empty($addressErr)) ? "{$addressErr}" : "Ex:
                           XYY Main Street" ?></small>
                </div>
            </div>
